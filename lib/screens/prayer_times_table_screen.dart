...
class PrayerTimesTableScreen extends StatelessWidget{const PrayerTimesTableScreen({super.key});
  @override Widget build(BuildContext c){final p=_P.of(Provider.of<AppProvider>(c).isDarkMode);final l=AppLocalizations.of(c)!;
    return Scaffold(backgroundColor:p.bg,appBar:AppBar(backgroundColor:Colors.transparent,elevation:0,foregroundColor:p.fg,title:Text(l.translate('prayerTimesTable'),style:TextStyle(fontSize:15,color:p.muted)),centerTitle:true),
      body:Center(child:Text('Prayer Times Table',style:TextStyle(fontSize:18,color:p.fg))));}}
