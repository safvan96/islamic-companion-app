...
class NightRoutineScreen extends StatefulWidget{const NightRoutineScreen({super.key});@override State<NightRoutineScreen>createState()=>_NightRoutineScreenS();}
class _NightRoutineScreenS extends State<NightRoutineScreen>{Set<int>_checked={};
  @override void initState(){super.initState();_load();}
  Future<void>_load()async{final p=await SharedPreferences.getInstance();final r=p.getStringList('nightRoutine_checked');if(r!=null)_checked=r.map((s)=>int.tryParse(s)??-1).where((i)=>i>=0).toSet();if(!mounted)return;setState((){});}
  Future<void>_save()async{final p=await SharedPreferences.getInstance();await p.setStringList('nightRoutine_checked',_checked.map((e)=>e.toString()).toList());}
  ...
  @override Widget build(BuildContext c){final p=_P.of(Provider.of<AppProvider>(c).isDarkMode);final l=AppLocalizations.of(c)!;
    final items=["nr_1","nr_2","nr_3","nr_4","nr_5","nr_6"];
    final progress=items.isEmpty?0.0:_checked.length/items.length;
    return Scaffold(backgroundColor:p.bg,appBar:AppBar(backgroundColor:Colors.transparent,elevation:0,scrolledUnderElevation:0,foregroundColor:p.fg,title:Text(l.translate('nightRoutine'),style:TextStyle(fontSize:15,fontWeight:FontWeight.w500,color:p.muted)),centerTitle:true),
