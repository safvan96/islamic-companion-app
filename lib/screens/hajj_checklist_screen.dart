...
class _HajjChecklistScreenS extends State<HajjChecklistScreen>{Set<int>_checked={};
  ...
  Future<void>_load()async{final p=await SharedPreferences.getInstance();final r=p.getStringList('hajjChecklist_checked');if(r!=null)_checked=r.map((s)=>int.tryParse(s)??-1).where((i)=>i>=0).toSet();if(!mounted)return;setState((){});}
